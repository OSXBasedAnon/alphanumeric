...
async fn async_main() -> Result<()> {
    ...
    let db_path = {
        ...
        if p.is_absolute() {
            ...
            // A candidate mid-rotation MUST win before any probe: in the
            // between-renames crash state its live dir is absent, so the
            // genesis/block probes below would report "no data" and silently
            // resolve to the OTHER candidate — booting a different database
            // while the interrupted rotation strands forever. Journal presence
            // is a pure existence check (no DB open); the startup repair hook
            // then heals the layout before anything reads it.
            let cwd_mid_rotation = rotation_paths(&cwd_str).journal.exists();
            let exe_mid_rotation = rotation_paths(&exe_str).journal.exists();

            if cwd_mid_rotation {
                cwd_str
            } else if exe_mid_rotation {
                exe_str
            } else {
    ...
    local.run_until(async move {
        ...
        // Heal any rotation interrupted mid-swap BEFORE anything reads the DB
        // directory; an unrepairable layout must stop the launch, not be
        // half-opened.
        match repair_interrupted_db_rotation(&db_path) {
            Ok(RotationRepair::NoJournal) => {}
            Ok(outcome) => boot_note(Some(&pb), format!("DB rotation repaired: {:?}", outcome)),
            Err(e) => {
...
println!("Wallet renamed successfully");
                ...
                Some("mine") => {
                    let parts: Vec<&str> = command.split_whitespace().collect();
                    let is_flag = |s: &str| matches!(s, "--continuous" | "-c");
...
// ---------------------------------------------------------------------------
// Offline verified DB rebuild + crash-safe rotation (memory-plan Release D).
//
// sled never returns physical disk: logical deletes leave the heap file at its
// high-water mark forever. The reclamation that is safe for a consensus DB is a
...
const ROTATION_JOURNAL_MAGIC: &[u8] = b"alphanumeric-rotation-v1\n";

/// `open_chain_db_aux` with a bounded wait on the sled directory lock. Dropping
/// a sled handle releases its flock asynchronously (background threads), so the
/// rebuild's reopen of a just-closed DB — and a rebuild started the moment a
/// service manager stopped the node — can transiently see WouldBlock. Anything
/// still holding the lock after the wait is a genuinely running process.
fn open_chain_db_aux_waiting(
    db_path: &str,
    attempts: u32,
) -> std::result::Result<sled::Db, sled::Error> {
    // sled wraps the flock failure in an Io error that is not always
    // WouldBlock-kinded (a custom "could not acquire lock" message), so match
    // both shapes.
    let lock_contention = |e: &sled::Error| match e {
        sled::Error::Io(io) => {
    ...
    for _ in 0..attempts {
        match open_chain_db_aux(db_path) {
            Ok(db) => return Ok(db),
            Err(e) if lock_contention(&e) => {
                last_err = Some(e);
    ...
    Err(last_err.unwrap_or_else(|| {
        sled::Error::Io(std::io::Error::new(
            ...
            "database directory lock held",
        ))
    }))
}

struct RotationPaths {
    live: std::path::PathBuf,
    rebuilt: std::path::PathBuf,
    retired: std::path::PathBuf,
    journal: std::path::PathBuf,
}

fn rotation_paths(db_path: &str) -> RotationPaths {
    // Normalize a trailing separator: with it, the sibling paths would land
    // INSIDE the live directory and the first rename would fail post-commit.
    let normalized = db_path.trim_end_matches('/');
    let normalized = if normalized.is_empty() {
        db_path
    } else {
        normalized
    };
    let live = std::path::PathBuf::from(normalized);
    ...
    RotationPaths {
        rebuilt: suffixed(".rebuilt"),
        retired: suffixed(".pre-rebuild"),
        journal: suffixed(".rotation-journal"),
        live,
    }
}

/// Directory-entry durability: renames and unlinks are only crash-durable once
/// the parent directory itself is fsynced.
fn fsync_parent_dir(path: &std::path::Path) -> std::result::Result<(), String> {
    let parent = path.parent().filter(|p| !p.as_os_str().is_empty());
    let parent = parent.unwrap_or_else(|| std::path::Path::new("."));
    std::fs::File::open(parent)
        .and_then(|f| f.sync_all())
        .map_err(|e| format!("fsync of {} failed: {}", parent.display(), e))
}

#[derive(Debug, PartialEq, Eq)]
enum RotationRepair {
    NoJournal,
    /// Crash before the first rename (or the work dir is gone): the live DB was
    /// never touched. The journal is cleared and the old database stays.
    RolledBackKeptLive,
    /// Crash between the renames: the rebuilt DB (already fully verified before
    /// the journal was written) is renamed into place.
    RolledForwardToRebuilt,
    /// Crash after both renames, before the journal was removed.
    ClearedFinishedJournal,
}

/// Deterministic repair of an interrupted rotation, decided purely from which
/// directories exist. Every reachable crash state maps to exactly one whole
/// database; anything else (operator intervention mid-swap) fails closed.
fn repair_interrupted_db_rotation(db_path: &str) -> std::result::Result<RotationRepair, String> {
    let paths = rotation_paths(db_path);
    if !paths.journal.exists() {
        return Ok(RotationRepair::NoJournal);
    }
    let clear_journal = |paths: &RotationPaths| -> std::result::Result<(), String> {
        std::fs::remove_file(&paths.journal)
            .map_err(|e| format!("failed to remove rotation journal: {}", e))?;
        fsync_parent_dir(&paths.journal)
    };
    match (
        paths.live.is_dir(),
        paths.rebuilt.is_dir(),
        paths.retired.is_dir(),
    ) {
        // Journal written, live untouched. The rebuilt dir may exist (crash
        // before rename 1) or have been lost; either way the safe whole
        // database is the live one.
        (true, _, false) => {
            clear_journal(&paths)?;
            Ok(RotationRepair::RolledBackKeptLive)
        }
        // Between the renames: live has moved to retired, rebuilt is verified
        // and waiting. Finish the swap.
        (false, true, true) => {
            std::fs::rename(&paths.rebuilt, &paths.live)
                .map_err(|e| format!("failed to complete interrupted rotation: {}", e))?;
            fsync_parent_dir(&paths.live)?;
            clear_journal(&paths)?;
            Ok(RotationRepair::RolledForwardToRebuilt)
        }
        // Both renames done; only the journal removal was lost.
        (true, false, true) => {
            clear_journal(&paths)?;
            Ok(RotationRepair::ClearedFinishedJournal)
        }
        (live, rebuilt, retired) => Err(format!(
            "rotation journal present but the directory layout is not a known crash state \
             (live={}, rebuilt={}, retired={}). Refusing to guess: inspect {} / {} / {} — \
             the retired directory, if present, holds the pre-rebuild database",
            live,
            rebuilt,
            retired,
            paths.live.display(),
            paths.rebuilt.display(),
            paths.retired.display()
        )),
    }
}

/// Per-tree logical digest: entry count plus a SHA-256 over every length-prefixed
/// key/value in iteration order, for every tree in the DB. Two databases with
/// equal digest maps hold identical logical content. Keys are the RAW tree-name
/// bytes so two distinct non-UTF-8 names can never fold into one map entry.
fn sled_tree_digests(
    db: &sled::Db,
) -> std::result::Result<std::collections::BTreeMap<Vec<u8>, (u64, String)>, String> {
    let mut out = std::collections::BTreeMap::new();
    for name in db.tree_names() {
        let tree = db
            .open_tree(&name)
            .map_err(|e| format!("open_tree failed: {}", e))?;
        let mut hasher = Sha256::new();
        let mut count: u64 = 0;
        for kv in tree.iter() {
            let (k, v) = kv.map_err(|e| format!("tree scan failed: {}", e))?;
        ...
        out.insert(name.to_vec(), (count, hex::encode(hasher.finalize())));
    }
    Ok(out)
}
...
/// The offline rebuild driver. The live database is not modified until the
/// rebuilt copy has passed full logical verification; the commit point is the
/// journal write, and every step after it is completable by
/// `repair_interrupted_db_rotation`.
fn run_offline_db_rebuild(db_path: &str) -> std::result::Result<(), String> {
    match repair_interrupted_db_rotation(db_path)? {
        RotationRepair::NoJournal => {}
        outcome => println!("Repaired an interrupted rotation first: {:?}", outcome),
    }
    let paths = rotation_paths(db_path);
    if paths.retired.exists() {
        return Err(format!(
            "a previous pre-rebuild backup still exists at {} — confirm the node is healthy \
             on the rebuilt database, remove or archive that backup, then re-run",
            paths.retired.display()
        ));
    }
    if !paths.live.is_dir() {
        return Err(format!("no database directory at {}", paths.live.display()));
    }
    if paths.rebuilt.exists() {
        std::fs::remove_dir_all(&paths.rebuilt)
            .map_err(|e| format!("failed to clear stale work dir: {}", e))?;
    }

    let before_bytes = dir_size_bytes(&paths.live);
    println!(
        "Rebuilding {} ({} MiB physical) into {} …",
        paths.live.display(),
        before_bytes / (1024 * 1024),
        paths.rebuilt.display()
    );

    let src = open_chain_db_aux_waiting(db_path, 20)
        .map_err(|e| format!("failed to open source DB (is the node stopped?): {}", e))?;
    let src_digests = sled_tree_digests(&src)?;
    let dst = sled::Config::new()
        .path(&paths.rebuilt)
        .cache_capacity(AUX_DB_CACHE_BYTES)
        .open()
        .map_err(|e| format!("failed to create rebuilt DB: {}", e))?;
    dst.import(src.export());
    dst.flush()
        .map_err(|e| format!("flush of rebuilt DB failed: {}", e))?;
    let dst_digests = sled_tree_digests(&dst)?;
    let after_bytes = dir_size_bytes(&paths.rebuilt);
    drop(dst);
    drop(src);
    // Durability of the rebuilt DB's own dirents (db/conf/blobs) AND of its entry
    // in the parent, both BEFORE the commit point: sled itself never fsyncs
    // directories, and on a strict-POSIX filesystem a power loss after the
    // renames could otherwise roll a hollow directory forward.
    std::fs::File::open(&paths.rebuilt)
        .and_then(|f| f.sync_all())
        .map_err(|e| format!("fsync of {} failed: {}", paths.rebuilt.display(), e))?;
    fsync_parent_dir(&paths.rebuilt)?;

    if src_digests != dst_digests {
        let first_diff = src_digests
            .iter()
            .find(|(name, digest)| dst_digests.get(*name) != Some(digest))
            .map(|(name, _)| name.clone())
            .or_else(|| {
                dst_digests
                    .keys()
                    .find(|name| !src_digests.contains_key(*name))
                    .cloned()
            })
            .map(|name| String::from_utf8_lossy(&name).into_owned())
            .unwrap_or_else(|| "<unknown>".to_string());
        let _ = std::fs::remove_dir_all(&paths.rebuilt);
        return Err(format!(
            "verification FAILED (first differing tree: {}). The live database was not \
             modified; the rebuilt copy has been discarded",
            first_diff
        ));
    }
    println!(
        "Verified: {} trees, logical content identical. Physical: {} MiB -> {} MiB.",
        src_digests.len(),
        before_bytes / (1024 * 1024),
        after_bytes / (1024 * 1024)
    );

    // COMMIT POINT. From here every crash state is repaired deterministically.
    {
        let mut journal = std::fs::File::create(&paths.journal)
            .map_err(|e| format!("failed to write rotation journal: {}", e))?;
        journal
            .write_all(ROTATION_JOURNAL_MAGIC)
            .and_then(|_| journal.sync_all())
            .map_err(|e| format!("failed to sync rotation journal: {}", e))?;
    }
    fsync_parent_dir(&paths.journal)?;
    std::fs::rename(&paths.live, &paths.retired)
        .map_err(|e| format!("rename live -> retired failed: {}", e))?;
    fsync_parent_dir(&paths.live)?;
    std::fs::rename(&paths.rebuilt, &paths.live)
        .map_err(|e| format!("rename rebuilt -> live failed: {}", e))?;
    fsync_parent_dir(&paths.live)?;
    std::fs::remove_file(&paths.journal)
        .map_err(|e| format!("failed to clear rotation journal: {}", e))?;
    fsync_parent_dir(&paths.journal)?;

    // Post-swap reopen check: the rotated-in directory opens and shows the exact
    // verified tree set. The operator's healthy node restart is the final gate.
    // Generous lock wait: our own just-dropped handle on a large DB can release
    // its flock slowly; the on-disk state is already complete at this point.
    let reopened = open_chain_db_aux_waiting(db_path, 100)
        .map_err(|e| format!("rotated-in DB failed to reopen: {}", e))?;
    let reopened_names: std::collections::BTreeSet<Vec<u8>> = reopened
        .tree_names()
        .into_iter()
        .map(|n| n.to_vec())
        .collect();
    let expected_names: std::collections::BTreeSet<Vec<u8>> = src_digests.keys().cloned().collect();
    if reopened_names != expected_names {
        return Err(format!(
            "rotated-in DB reopened with an unexpected tree set — the pre-rebuild database \
             is retained at {}",
            paths.retired.display()
        ));
    }
    println!(
        "Rotation complete. Reclaimed {} MiB. The pre-rebuild database is retained at {} — \
         start the node, confirm it is healthy, then remove that directory.",
        before_bytes.saturating_sub(after_bytes) / (1024 * 1024),
        paths.retired.display()
    );
    Ok(())
}
...
mod tests {
    ...
    fn rotation_fixture(tag: &str) -> (std::path::PathBuf, String) {
        let base = std::env::temp_dir().join(format!("an-rotation-{}-{}", std::process::id(), tag));
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(&base).expect("fixture dir");
        let live = base.join("chain.db");
        (base, live.to_string_lossy().into_owned())
    }

    // Every reachable crash state of the two-rename swap must repair to exactly
    // one whole database; unrecognized layouts must refuse rather than guess.
    #[test]
    fn rotation_repair_selects_a_whole_database_at_every_crash_point() {
        let (base, db) = rotation_fixture("repair");
        let paths = rotation_paths(&db);
        let mkdirs = |live: bool, rebuilt: bool, retired: bool, journal: bool| {
            for p in [&paths.live, &paths.rebuilt, &paths.retired] {
                let _ = std::fs::remove_dir_all(p);
            }
            let _ = std::fs::remove_file(&paths.journal);
            if live {
                std::fs::create_dir_all(&paths.live).unwrap();
            }
            if rebuilt {
                std::fs::create_dir_all(&paths.rebuilt).unwrap();
                std::fs::write(paths.rebuilt.join("marker"), b"rebuilt").unwrap();
            }
            if retired {
                std::fs::create_dir_all(&paths.retired).unwrap();
            }
            if journal {
                std::fs::write(&paths.journal, ROTATION_JOURNAL_MAGIC).unwrap();
            }
        };

        // No journal: never touches anything.
        mkdirs(true, true, false, false);
        assert_eq!(
            repair_interrupted_db_rotation(&db).unwrap(),
            RotationRepair::NoJournal
        );
        assert!(paths.live.is_dir() && paths.rebuilt.is_dir());

        // Crash before rename 1: rollback, live untouched, journal cleared, and
        // the (inert) work dir left in place for the next run to clear.
        mkdirs(true, true, false, true);
        assert_eq!(
            repair_interrupted_db_rotation(&db).unwrap(),
            RotationRepair::RolledBackKeptLive
        );
        assert!(paths.live.is_dir() && !paths.journal.exists());
        assert!(
            paths.rebuilt.is_dir(),
            "rollback never deletes the work dir"
        );

        // A torn journal (created, crash before its bytes landed) is only
        // reachable pre-rename: repair must treat it exactly like a full one.
        mkdirs(true, true, false, false);
        ...
        // Same rollback when the work dir was also lost.
        mkdirs(true, false, false, true);
        assert_eq!(
            repair_interrupted_db_rotation(&db).unwrap(),
            RotationRepair::RolledBackKeptLive
        );

        // Crash between the renames: roll forward — the verified rebuilt DB
        // becomes live.
        mkdirs(false, true, true, true);
        assert_eq!(
            repair_interrupted_db_rotation(&db).unwrap(),
            RotationRepair::RolledForwardToRebuilt
        );
        assert!(
            paths.live.join("marker").exists(),
            "the rebuilt content is now the live directory"
        );
        assert!(!paths.rebuilt.exists() && !paths.journal.exists());

        // Crash after both renames: only the journal is cleared.
        mkdirs(true, false, true, true);
        assert_eq!(
            repair_interrupted_db_rotation(&db).unwrap(),
            RotationRepair::ClearedFinishedJournal
        );
        assert!(paths.live.is_dir() && paths.retired.is_dir() && !paths.journal.exists());

        // Unrecognized layouts fail closed.
        mkdirs(false, false, true, true);
        assert!(repair_interrupted_db_rotation(&db).is_err());
        mkdirs(true, true, false, false);
        std::fs::create_dir_all(&paths.retired).unwrap();
        std::fs::write(&paths.journal, ROTATION_JOURNAL_MAGIC).unwrap();
        assert!(repair_interrupted_db_rotation(&db).is_err());
    ...
    // The full offline driver: identical logical content after rotation, the
    // pre-rebuild database retained, and a leftover backup refuses a second run.
    #[test]
    fn offline_rebuild_rotates_and_preserves_every_tree() {
        let (base, db_path) = rotation_fixture("driver");
        {
            ...
            for i in 0..200u32 {
                db.insert(format!("block_{}", i).as_bytes(), vec![i as u8; 64])
                    .unwrap();
            }
            let tree = db.open_tree("witnesses").unwrap();
            for i in 0..50u32 {
                tree.insert(i.to_le_bytes(), vec![0xEE; 128]).unwrap();
            }
            // Deletions leave physical garbage for the rebuild to shed.
            for i in 0..100u32 {
                db.remove(format!("block_{}", i).as_bytes()).unwrap();
            }
            db.flush().unwrap();
            let expected = sled_tree_digests(&db).unwrap();
            // The directory lock is held until EVERY handle drops — the Tree
            // included — exactly the operator rule the driver's bounded lock
            // wait backstops.
            drop(tree);
            drop(db);

            run_offline_db_rebuild(&db_path).expect("rebuild succeeds");

            let paths = rotation_paths(&db_path);
            assert!(paths.retired.is_dir(), "pre-rebuild DB is retained");
            assert!(!paths.journal.exists(), "journal cleared on success");
            assert!(!paths.rebuilt.exists(), "work dir was rotated into place");
            let rotated = open_chain_db_aux(&db_path).expect("rotated DB opens");
            assert_eq!(
                sled_tree_digests(&rotated).unwrap(),
                expected,
                "every tree survives the rotation byte-identically"
            );
            drop(rotated);

            // The retained backup must itself be a whole, openable database
            // with the same content — it is the operator's rollback path.
            let backup = open_chain_db_aux_waiting(&paths.retired.to_string_lossy(), 20)
                .expect("retained pre-rebuild DB opens");
            assert_eq!(
                sled_tree_digests(&backup).unwrap(),
                expected,
                "the retained backup is the intact pre-rebuild database"
            );
            drop(backup);

            // A retained backup blocks the next run until the operator acts.
            let err = run_offline_db_rebuild(&db_path).unwrap_err();
            assert!(
                err.contains("pre-rebuild backup"),
    ...
    // The digest map must distinguish databases that differ in any tree.
    #[test]
    fn tree_digests_detect_content_differences() {
        let (base, db_path) = rotation_fixture("digest");
        let a = sled::Config::new()
            .path(std::path::Path::new(&db_path))
            .cache_capacity(64 * 1024 * 1024)
            .open()
            .unwrap();
        a.insert(b"k", b"v1").unwrap();
        let da = sled_tree_digests(&a).unwrap();
        a.insert(b"k", b"v2").unwrap();
        let db2 = sled_tree_digests(&a).unwrap();
        assert_ne!(da, db2, "a single changed value changes the digest");
        drop(a);
        let _ = std::fs::remove_dir_all(&base);
    }
