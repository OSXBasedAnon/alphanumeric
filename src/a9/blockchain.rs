...
mod tests {
    ...
    async fn racing_miners_with_pending_tx_both_complete() {
        ...
        let task_a = tokio::spawn(async move {
            let mut h = ha;
            mgr_a
                .mine_block(&mut h, &ta, 1u64 << 26, "miner_a".to_string())
                .await
        });
        let task_b = tokio::spawn(async move {
            let mut h = hb;
            mgr_b
                .mine_block(&mut h, &tb, 1u64 << 26, "miner_b".to_string())
                .await
        });
    ...
    async fn racing_miners_both_complete_loser_recovers() {
        ...
        let task_a = tokio::spawn(async move {
            let mut h = ha;
            mgr_a
                .mine_block(&mut h, &txs_a, 1u64 << 26, "miner_a".to_string())
                .await
        });
        let task_b = tokio::spawn(async move {
            let mut h = hb;
            mgr_b
                .mine_block(&mut h, &txs_b, 1u64 << 26, "miner_b".to_string())
                .await
        });
    ...
    async fn mines_a_valid_block_at_min_difficulty() {
        ...
        let (nonce, hash_string, block) = tokio::time::timeout(
            ...
            manager.mine_block(&mut header, &no_txs, 1u64 << 27, "miner_proof".to_string()),
        )
        .await
